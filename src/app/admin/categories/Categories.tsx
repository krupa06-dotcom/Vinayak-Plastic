...
// Categories — port of src/pages/admin/categories/index.astro. The heavy
// lifting is DOM rendering started after gate() resolves, exactly like the
// Astro script.
export default function Categories() {
  useEffect(() => {
    void (async () => {
      ...
      interface SubRow { id: string; category_id: string; is_active: boolean }
      ...
      async function load() {
        ...
        const [catsRes, subsRes] = await Promise.all([
          ...
          supabase.from('sub_categories').select('id, category_id, is_active')
        ]);
        ...
        if (!cats.length) {
          showEmpty(listEl, 'No categories yet', 'Categories are the main ranges shown first on the Products page. Create your first category to get started.',
            `<button type="button" class="a-btn a-btn-primary" data-cat-openform>+ Add Category</button>`);
          listEl.querySelector<HTMLButtonElement>('[data-cat-openform]')?.addEventListener('click', openForm);
          return;
        }
        ...
        listEl.innerHTML = `
          <div class="a-table-wrap">
            <table class="a-table">
              <thead><tr>
                <th>Category</th><th>Contents</th><th style="width:110px">Status</th><th style="width:230px">Actions</th>
              </tr></thead>
              <tbody>
                ${cats.map(c => {
    ...
    <AdminShell title="Categories" current="categories">
      <div className="a-page-head">
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <a href="/admin/products/" className="a-btn">← Back</a>
          <button type="button" className="a-btn a-btn-primary" id="add-cat-toggle">+ Add Category</button>
        </div>
      </div>

      <div className="a-card" id="add-cat-form" style={{ display: 'none' }}>
          ...
          <form id="cat-form" noValidate>
            <div className="a-form-grid">
              ...
              <div className="a-field a-field-full">
                ...
                <textarea id="cat-description" className="a-textarea" rows={3} placeholder="What this range is and where it is used"></textarea>
              </div>
              <div className="a-field">
              ...
              <div className="a-field">
                <label>{'\u00A0'}</label>
                <label className="a-check"><input type="checkbox" id="cat-active" defaultChecked /> Active</label>
              </div>
            </div>
